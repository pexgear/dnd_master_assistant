...
# The venv's bin directory is only on PATH while the venv is activated, so
# `canonkeeper` alone will not be found. This wrapper makes ./canonkeeper work
# from the project folder, which is what people actually try.
step "Writing a launcher"
cat > canonkeeper <<LAUNCHER
#!/usr/bin/env bash
# Generated by install.sh -- runs Canon Keeper from this folder's virtualenv.
exec "\$(dirname "\$0")/$VENV_PATH/bin/canonkeeper" "\$@"
LAUNCHER
chmod +x canonkeeper
ok "./canonkeeper"
...
echo
echo "  Other ways in:"
echo "    ./$VENV_PATH/bin/canonkeeper                        the same thing, spelled out"
echo "    ./$VENV_PATH/bin/canonkeeper-server --db <campaign> headless host"
echo "    F5 in VS Code, picking 'Canon Keeper'"
