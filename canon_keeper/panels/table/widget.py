...
from canon_keeper.panels.table.dialogs import HostDialog, JoinDialog
...
class TableWidget(QWidget):
    def __init__(self, ctx: AppContext) -> None:
        ...
        self._client = SessionClient(self)
    ...
    def _build_ui(self) -> None:
        ...
        bar = QHBoxLayout()
        self._host_button = QPushButton("Host session")
    ...
    def _host(self) -> None:
        dialog = HostDialog(self._default_name(), self)
        if not dialog.exec():
            return
        name, port, session_name = dialog.values()

        server = SessionServer(session_name, parent=self)
        ...
        self._append_system(
            f"Hosting {session_name!r} on port {server.port}. Join code: {server.code}"
        )
        # Join our own server like anyone else, so there is one code path.
        self._client.join(
            f"ws://127.0.0.1:{server.port}", server.code, name, Role.DM.value
        )
        self._update_state()

    def _join(self) -> None:
        dialog = JoinDialog(
            self._ctx.repos.settings.get("session_name", "") or "Player",
            self._ctx.repos.settings.get("session_last_url", ""),
            self,
        )
        if not dialog.exec():
            return
        url, code, name = dialog.values()

        self._ctx.repos.settings.set("session_name", name)
        self._ctx.repos.settings.set("session_last_url", url)
        role = Role.DM.value if self._ctx.role == Role.DM.value else Role.PLAYER.value
        self._append_system(f"Connecting to {url}...")
        self._client.join(url, code, name, role)
        self._update_state()
    ...
    def _on_roster(self, members: list[Member]) -> None:
        ...
        for member in members:
            item = QListWidgetItem(
                f"{member.name}  ({'DM' if member.role == Role.DM.value else 'player'})"
            )
    ...
    def _on_said(self, member: Member, text: str) -> None:
        self._append(member.role, f"{member.name}: {text}")

    def _on_rolled(self, member: Member, payload: dict) -> None:
        self._append("roll", f"{member.name} rolled {payload.get('description', '')}")
    ...
    def _update_state(self) -> None:
        ...
        self._host_button.setEnabled(not connected and not hosting)
        ...
        if hosting and self._server is not None:
            self._status.setText(
                f"Hosting on port {self._server.port} - join code "
                f"{self._server.code} - players on this network will see it listed."
            )
        elif connected:
            me = self._client.me
