...
def describe_fight(fight: dict, entities: dict) -> str:
    ...
    for combatant in fight.get("combatants") or []:
        ...
        if combatant.get("down"):
            ...
            spot += ", down"
        if combatant.get("simulated") and combatant.get("stand_in"):
            # Somebody is sitting in that seat and will answer a proposal, so
            # this one is played the way a person's character is played.
            spot += ", a stand-in speaks for them"
        elif combatant.get("simulated"):
            # Handed over with nothing there to answer. Take the turn, or the
            # fight stops on a proposal nobody can accept.
            spot += ", played by you"
